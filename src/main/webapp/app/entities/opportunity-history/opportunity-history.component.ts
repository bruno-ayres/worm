...
export class OpportunityHistoryComponent implements OnInit, OnDestroy {
    ...
    loadAll() {
        this.opportunityHistoryService
            ...
            .subscribe(
                (res: HttpResponse<IOpportunityHistory[]>) => this.paginateOpportunityHistories(res.body, res.headers),
    ...
    transition() {
        this.router.navigate(['/opportunity-history'], {
            queryParams: {
                page: this.page,
                size: this.itemsPerPage,
    ...
    trackId(index: number, item: IOpportunityHistory) {
        return item.id;
    }

    registerChangeInOpportunityHistories() {
        this.eventSubscriber = this.eventManager.subscribe('opportunityHistoryListModification', response => this.loadAll());
    }
    ...
    protected paginateOpportunityHistories(data: IOpportunityHistory[], headers: HttpHeaders) {
        this.links = this.parseLinks.parse(headers.get('link'));
        this.totalItems = parseInt(headers.get('X-Total-Count'), 10);
        this.queryCount = this.totalItems;
        this.opportunityHistories = data;
    }
